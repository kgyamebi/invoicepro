import { convertInvoiceToReceipt, serializeDocument } from "@/server/documents";
...
export async function POST(_request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const receipt = await convertInvoiceToReceipt(id, access.user.id, access.organization.id);
